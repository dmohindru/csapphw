...
void mm_free(void *bp)
{
    ...
    /* First set header of current block */
    PUT(HDRP(bp), PACK(size, 0, GET_PREV_ALLOC(bp)));
    //PUT(FTRP(bp), PACK(size, 0, GET_PREV_ALLOC(bp)));
    /* Get the next block and see if its allocated, then
       only update its header, else update its header and
       footer */
...
/* $end mmplace-proto */
{
    ...
    if ((csize - asize) >= (DSIZE)) { 
        ...
        PUT(HDRP(NEXT_BLKP(bp)), PACK(csize, GET_ALLOC(HDRP(NEXT_BLKP(bp))), 1));
    }
}
/* $end mmplace */
