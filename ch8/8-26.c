...
typedef struct process_entry
{
	...
	char run_status[20];	 /* Run status of process */
...
process_entry_t *find_job(char *job_id); /* Function to find job from global list */
void display_jobs(); /* Function to display jobs */
void do_bgfg(int fgbg); /* Function to process bg and fg command */
...
void display_jobs()
{
	...
	while(entry)
	{
		printf("[%d] %d %s\t %s", entry->job_id, entry->process_id, entry->run_status, entry->cmdline);
		entry = entry->next;
	}
	//printf("Displaying jobs\n");
}

void do_bgfg(int fgbg)
{
	if (fgbg == BG) {
		printf("Processing background jobs\n");
	}
	else if(fgbg == FG) {
		printf("Processing foreground jobs\n");
	}
}
